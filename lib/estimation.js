...
var CML = function (myData, myItems,myTrueItems, counter) {
    ...
    var refCatSet = false;
    var ranked = _.findWhere(myItems, function(itm){
    ...
    for (var j = 0; j < myItems.length; j++) {//for1   //loop through the items
        if(myItems[j].type == "to rank") { //if1          //only the item "to rank" should be estimated (This is generic because if you don't have a previous ranking, all items will be "to rank"
            ...
            if (selected.comparedNum > 0) {//if4     //if selected has already been compared
                ...
                var info = _.reduce(opponents, function (memo, num) {//reduce2
                    return memo + Stats.fischerI(myTrueItems[j].ability.value, num.ability.value);
                }, 0);//end reduce2
                if (counter > 0) {//if5    //as long as all estimation iteration aren't over
                    myItems[j].ability.value += ((obsScore - expectScore) / info); //estimate ability
                } else {//end if5    //otherwise
                    myItems[j].ability.se = 1 / Math.sqrt(info);     //estimate ability se
                }//end else
            }//end if4
        }//end if1
    }//end for1
