 ...
 * @property {string} itemA - the ID of the "A" item
 * @property {string} itemB - the ID of the "B" item
...
/**
 * retrieve a VO from the lookup lookup, or create one and store it
 * @private
 * @param {Lookup} lookup
 * @param {string} id
 * @returns {ValueObject}
  ...
  estimate: function estimate(comparisons,
    ...
    comparisons.forEach((comparison) => {
      if (comparison.selected) { // we only want to take "finished" comparisons into account
        ...
        prepValuesOnFirstComparison(lookup, comparison.itemA);
        prepValuesOnFirstComparison(lookup, comparison.itemB);
      }
    });
    ...
    return Object.values(lookup.unrankedById);
  }
};