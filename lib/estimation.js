...
function CML(lookup,
  ...
  Object.keys(lookup.unrankedById)
      .forEach((id) => {
        ...
        if (iteration > 0) {
          const incr = Math.min((current.selectedNum - expected.value) / expected.info, maxIncr)
...
module.exports = {
   ...
   * @param {Comparison[]} payload.comparisons
   * @param {Item[]|{}} payload.items - An Array or a map of {@link Item}
   * @returns {Item[]|{}} An Array or a map of {@link Item} (reflects the type of parameter `items`)
   */
  estimate: function estimate(payload) {
