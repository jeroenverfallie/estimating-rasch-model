...
var estimateCJ = function (rawData, items) {
    // initially set estimates of items "to rank" to 0
    _.forEach(items, function (item) {     //loop through items
        if(item.type=="to rank"){       //only the items "to rank" need to be estimated
    ...
    var Data = restructure(rawData);

    //loop 4 times (+1) through the estimation
    for (var i = 4; i >= 0; i--) {
        CML(Data, items, items.slice(0), i);    //items.slice(0) because we need to acces the ability estimates from the
...
var CML = function (myData, myItems,myTrueItems, counter) {
    ...
    for (var i = 0; i < myItems.length; i++) {//for1   //loop through the items
        if(myItems[i].type=="to rank") { //if1          //only the item "to rank" should be estimated (This is generic because if you don't have a previous ranking, all items will be "to rank"
            ...
            var oppos = selected.decisions;           //save the opponent id's
            ...
            if (selected.comparedNum > 0) {//if4     //if selected has already been compared
                ...
                //Calculate info
                var info = _.reduce(oppontents, function (memo, num) {//reduce2
                ...
                if (counter > 0) {//if5    //as long as all estimation iteration aren't over
                    myItems.ability.value += ((obsScore - expectScore) / info); //estimate ability
                } else {//end if5    //otherwise
                    myItems.ability.se = 1 / Math.sqrt(info);     //estimate ability se
                }//end else
            }//end if4
        }//end if1
    }//end for1
